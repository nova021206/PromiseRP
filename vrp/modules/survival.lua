        ...
        vRPclient.isInComa(nplayer,{}, function(in_coma)
          if in_coma then
            if vRP.tryGetInventoryItem(user_id,"medkit",1,true) then
              ...
              local amount = 300000
              local emsamount = 400000
              local hiprice = 100000
              
              local money = vRP.getBankMoney(nuser_id)
              vRPclient.varyHealth(nplayer,{30}) -- heal 50
              vRP.setBankMoney(nuser_id,money-amount)
              MySQL.execute("vRP/delete_hi", {hi = hiprice})
              vRP.giveBankMoney(user_id,emsamount)
              vRPclient.notify(player,{"치료비를 받았습니다. ~g~300,000$"})
              vRPclient.notifyPicture(player,{"CHAR_BANK_MAZE",1,"건강보험공단",false,"치료비 추가지급: ~g~$100,000"})
              vRPclient.notify(nplayer,{"치료비를 지불했습니다. ~r~300,000$"})
              end)
            end
          else
