      ...
      vRP.generateRegistrationNumber(function(registration)
        vRP.generatePhoneNumber(function(phone)
          MySQL.execute("vRP/init_user_identity", {
            user_id = user_id,
            registration = vRP.generateStringNumber("LLLDDD"),
            phone = vRP.generateStringNumber("010DDDDDDDD"),
                ...
                if vRP.tryPayment(user_id,cfg.new_identity_cost) then
                  vRP.generateRegistrationNumber(function(registration)
                    vRP.generatePhoneNumber(function(phone)
                      ...
                      MySQL.execute("vRP/update_user_identity2", {
                        user_id = user_id,
                        firstname = firstname,
                        name = name,
                        age = age,
    ...
    vRP.getUserIdentity(user_id, function(identity)
      ...
      if identity then
        ...
        vRP.getUserAddress(user_id, function(address)
          ...
          local content = lang.cityhall.menu.info({htmlEntities.encode(identity.name),htmlEntities.encode(identity.firstname),identity.age,identity.registration,identity.phone,home,number,CR2,exp})
          local choices = {}
