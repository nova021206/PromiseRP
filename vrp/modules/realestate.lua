...
MySQL.createCommand("vRP/get_party_realestate","SELECT party FROM elysium_realestates WHERE realestate = @realestate")
--[[
...
function vRP.getUserRealestate(user_id, cbr)
  ...
  MySQL.query("vRP/get_realestate", {user_id = user_id}, function(rows,affected)
    task({rows[1]})
  end)
end
...
function vRP.getPartyRealestate(realestate, cbr)
  local task = Task(cbr)
  MySQL.query("vRP/get_party_realestate", {realestate = realestate}, function(rows,affected)
    task({rows[1].party})
  end)
end
...
function vRP.getUserByRealestate(realestate,number,cbr)
  ...
  MySQL.query("vRP/get_realestate_owner", {realestate = realestate, number = number}, function(rows, affected)
    if #rows > 0 then
      task({rows[1].user_id})
    else
      task()
...
function vRP.findRealestateFreeNumber(realestate,max,cbr)
  ...
  local function search()
    vRP.getUserByRealestate(realestate,i,function(user_id)
...
local function build_entry_menu(user_id, realestate_name)
  ...
  menu["부동산 구매"] = {function(player,choice)
    vRP.getUserRealestate(user_id, function(address)
      if address == nil then -- check if not already have a home
        vRP.findRealestateFreeNumber(realestate_name, realestate.max, function(number)
          if number ~= nil then
            if vRP.tryPayment(user_id, realestate.buy_price) then
              ...
              else if vRP.hasPermission(user_id,"shh.realestate") then
                vRP.setUserRealestate(user_id, realestate_name, number, 2)
              else if vRP.hasPermission(user_id,"yakuza.realestate") then
  ...
  end, lang.home.buy.description({realestate.buy_price})}

  menu["소유중인 조직"] = {function(player,choice)
      vRP.getPartyRealestate(realestate_name, function(party)
          if party == 1 then
          vRPclient.notify(player,{"마피아가 소유 중 입니다."})
          else if party == 2 then
          vRPclient.notify(player,{"삼합회가 소유 중 입니다."})
          else if party == 3 then
          vRPclient.notify(player,{"시미즈카이가 소유 중 입니다."})
          else
          vRPclient.notify(player,{"조직이 소유하고 있지 않습니다."})
        end
      end
    end
    end)
  ...
  menu["부동산 판매"] = {function(player,choice)
    vRP.getUserRealestate(user_id, function(address)
      if address ~= nil and address.realestate == realestate_name then -- check if already have a home
        ...
        vRP.prompt(player,"부동산을 정말 판매하시겠습니까? 'Y' 'N'","",function(player,id)
                if id == "Y" then
  ...
  end, lang.home.sell.description({realestate.sell_price})}
