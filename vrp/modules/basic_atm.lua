...
local function atm_choice_depositcredit(player,choice)
  ...
  vRP.prompt(source,lang.atm.depositcredit.prompt(),"",function(player,v)
    play_atm_exit(player)
    ...
    if v > 0 then
      ...
      if user_id ~= nil then
        if vRP.tryDepositCredit(user_id,v) then
          vRPclient.notify(source,{lang.atm.depositcredit.deposited({v})})
        else
          vRPclient.notify(source,{lang.money.not_enough()})
        end
      end
...
local function atm_choice_moneytocredit(player,choice)
  ...
  vRP.prompt(source,lang.atm.moneytocredit.prompt(),"",function(player,v)
    play_atm_exit(player)
    ...
    if v > 0 then
      ...
      if user_id ~= nil then
        if vRP.tryMoneyToCredit(user_id,v) then
          vRPclient.notify(source,{lang.atm.moneytocredit.deposited({v})})
        else
          vRPclient.notify(source,{lang.money.not_enough()})
        end
      end
  ...
  vRP.prompt(source,lang.atm.withdraw.prompt(),"",function(player,v)
    ...
    if v > 0 then
      ...
      if user_id ~= nil then
        ...
        if vRP.tryWithdraw(user_id,math.ceil(v)) then
          vRPclient.notify(source,{lang.atm.withdraw.withdrawn({v})})
          --vRP.tryFullPayment(user_id,susu)
          vRPclient.notify(source,{"~r~출금 수수료 5,000$"})
          vRP.AddToCompany({1,tonumber(susu)})
        else
...
local function atm_choice_repayloan(player,choice)
  ...
  vRP.prompt(source,"얼마를 상환하시겠습니까? 현재 대출금:"..loan,"",function(player,v)
    play_atm_exit(player) --anim
    v = parseInt(v)
...
local function atm_enter()
  ...
  if user_id ~= nil then
    ...
    atm_menu[lang.atm.checkloanlimit.title()] = {function()end,lang.atm.checkloanlimit.bank({math.ceil(tonumber(vRP.getLoanLimit(user_id)))})}
    if loan == 0 then
    atm_menu[lang.atm.noloaninfo.title()] = {function()end,lang.atm.noloaninfo.bank({vRP.getLoan(user_id)})}
    else
    atm_menu[lang.atm.loaninfo.title()] = {function()end,lang.atm.loaninfo.bank({vRP.getLoan(user_id)})}
    end
